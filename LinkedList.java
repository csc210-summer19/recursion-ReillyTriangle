 ...
 * 
 *         This generic collection class uses a singly-linked data structure to
 *         store store elements. Many operations are already implemented using
 *         recursion. Use them as examples of public methods that send the first
 *         to private helper methods. The private helpers must use recursion
 *         instead of loops.
 * 
...
public class LinkedList<E extends Comparable<E>> {
	...
	public void removeAll(E el) {
		...
		if (size() == 1 && first.data.equals(el)) {
			first = null;
			n = 0;
			return;
		}
		...
		if (size() == 2 && first.data.equals(el)) {
			first = first.next;
			n = 1;
			removeAll(el);
			return;
		...
		Node previous = first;
		Node ref = first.next;
		removeAll(previous, ref, el);
	}

	private void removeAll(Node previous, Node ref, E el) {

		if (previous.data.equals(el)) {
			first = ref;
			previous = first;
			ref = first.next;
			n = n - 1;
		...
		if (ref.data.equals(el) && ref.next == null) {
			previous.next = null;
			n = n - 1;
			return;
		}
